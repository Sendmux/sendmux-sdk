...
function writeSurfaceFiles(surface, specPath) {
  const packageDir = join(root, "go", surface.name);
  patchConnectionInterfaceCompatibility(surface, packageDir);
...
function patchConnectionInterfaceCompatibility(surface, packageDir) {
  const operation = `${packageTitle(surface.name)}GetConnection`;
  const method = new RegExp(String.raw`\t// ${operation} (?:implements|invokes)[\s\S]*?\n\t${operation}\([^\n]+\n`);
  for (const file of ["oas_client_gen.go", "oas_server_gen.go"]) {
    replaceInFile(join(packageDir, file), method, "", `${surface.name} legacy interface connection method`);
  }
  replaceInFile(
    join(packageDir, "oas_handlers_gen.go"),
    new RegExp(String.raw`s\.h\.${operation}\(ctx, params\)`, "g"),
    "s.invokeConnection(ctx, params)",
    `${surface.name} optional connection handler dispatch`,
  );
  writeFileSync(
    join(packageDir, "connection.go"),
...
\t${operation}(ctx context.Context, params ${operation}Params) (${operation}Res, error)
}

func (s *Server) invokeConnection(ctx context.Context, params ${operation}Params) (${operation}Res, error) {
\tif handler, ok := s.h.(ConnectionInvoker); ok {
\t\treturn handler.${operation}(ctx, params)
\t}
\treturn UnimplementedHandler{}.${operation}(ctx, params)
}
`,
  );
}
