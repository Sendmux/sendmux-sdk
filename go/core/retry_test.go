...
func TestValidateAPIKey(t *testing.T) {
	...
	if err := ValidateAPIKey("smx_agent_example", KeySurfaceSending); err != nil {
		t.Fatalf("owner-approved agent token rejected for sending surface: %v", err)
	}
	...
	if err := ValidateAPIKey("smx_root_example", KeySurfaceSending); err == nil {
		t.Fatal("root key accepted for sending surface")
	}
}
