  ...
  class BatchUpdateMailboxMessagesBody < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Sendmux::Mailbox::Generated::BatchUpdateMailboxMessagesBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Sendmux::Mailbox::Generated::BatchUpdateMailboxMessagesBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'if_in_state')
        self.if_in_state = attributes[:'if_in_state']
      end

      if attributes.key?(:'keywords')
        if (value = attributes[:'keywords']).is_a?(Hash)
          self.keywords = value
        end
      end
    ...
    def hash
      [flagged, ids, if_in_state, keywords, seen].hash
    end
