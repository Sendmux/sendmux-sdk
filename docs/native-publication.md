...
For manually uploaded or promoted Snap revisions, verify the revision corresponds to the selected YAML version/checksum, then run `node scripts/publication-guard.mjs snap` from the current guard checkout with that exact Snap configuration immediately before upload or promotion. A prior green workflow is not a permanent certificate. Manual promotion remains owner-approved policy: this source change cannot prevent a credential holder from bypassing it with direct store or Git commands.
