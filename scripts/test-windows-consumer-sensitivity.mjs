...
for (const mutation of mutations) {
  ...
  console.log(JSON.stringify({ workspace: directory, owner_pid: process.pid, mutation: mutation.name }));
  let completed = false;
  try {
    ...
    await assert.rejects(run(process.execPath, [join(directory, "diagnose-windows-consumer-ownership.mjs"), evidence]), /exit 1/);
    ...
    completed = true;
    console.log(JSON.stringify({ sensitivity: mutation.name, detected: true, results: rows }));
  } finally {
    if (completed) {
