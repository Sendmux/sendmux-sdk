...
The guard compares complete immutable candidate-owned App and Sending schemas with fixed live endpoints before the first write in each supported release-workflow entry and the PHP split command. Canonical object-key order is the only normalisation: descriptions and arrays remain significant. Requests have one attempt, no redirect, a 15-second body-inclusive deadline and an 8 MiB decoded cap. Exact `release-please@17.6.0` supplies release construction; no duplicate version algorithm is introduced.
...
## Direct-helper boundary — 2026-09-17 08:27

Review 5228945381 correctly identified that directly invoking the Ruby writer can publish a local gem without checking provenance or live parity. The same limitation applies to the npm and Homebrew helpers. Their supported workflow callers are guarded before the first write; the helpers themselves are not guarded standalone release procedures. The README and publication guide now state that boundary explicitly instead of claiming every source-owned command is protected. No helper, workflow, runtime, or test behavior changes in this documentation correction. Standalone artifact-provenance enforcement and preventing credential-holder bypass remain outside this bounded workflow/PHP implementation; they are not claimed complete. Source trace and independent adjudication: `.claude/pr236-direct-writer-adjudication.md`.
