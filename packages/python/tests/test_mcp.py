...
def test_toolsets_are_curated_and_key_split() -> None:
    async def check() -> None:
        mailbox = create_server(ServerConfig(surface="mailbox", api_key="smx_mbx_test"), transport=ok_transport())
        management = create_server(ServerConfig(surface="management", api_key="smx_root_test"), transport=ok_transport())
        sending = create_server(ServerConfig(surface="sending", api_key="smx_root_test"), transport=ok_transport())
...
def test_wrong_key_prefix_rejected_before_server_start() -> None:
    with pytest.raises(ValueError):
        create_server(ServerConfig(surface="mailbox", api_key="smx_root_test"), transport=ok_transport())

    with pytest.raises(ValueError):
        create_server(ServerConfig(surface="management", api_key="smx_mbx_test"), transport=ok_transport())

    with pytest.raises(ValueError):
        create_server(ServerConfig(surface="sending", api_key="smx_mbx_test"), transport=ok_transport())
...
def test_mailbox_tool_call_injects_bearer_auth() -> None:
    ...
    async def check() -> None:
        server = create_server(
            ServerConfig(surface="mailbox", api_key="smx_mbx_test"),
...
def test_http_security_middleware_blocks_unauthorised_mcp_requests() -> None:
    ...
    async def check() -> None:
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://testserver") as client:
            ...
            assert health.json() == {"status": "ok", "surface": "mailbox"}
