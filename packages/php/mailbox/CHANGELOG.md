...
- Breaking: nullable object properties use dedicated `…OrNull` models that carry the same properties, getters and setters as their base models. `getFrom()` on `MailboxMessage`, `MailboxMessageSummary`, `MailboxRealtimeMessage` and `MailboxMessageContentParticipants` returns `?MailboxAddressOrNull`; `getLastMessage()` on `MailboxThread` and `MailboxThreadSummary` returns `?MailboxMessageSummaryOrNull`; `MailboxBatchGetItem::getContent()` returns `?MailboxMessageContentOrNull` and `getRawBody()` returns `?MailboxRawBodyOrNull`; `MailboxSubmissionEnvelope::getMailFrom()` returns `?MailboxSubmissionEnvelopeAddressOrNull` and `getRcptTo()` returns `MailboxSubmissionEnvelopeAddress[]`. `MailboxMessageContentResponse::getData()` returns `MailboxMessageContent`, `MailboxThreadContentResponse::getData()` returns `MailboxMessageContent[]` and `MailboxRawBodyResponse::getData()` returns `MailboxRawBody`; the `MailboxMessageContentResponseAllOfData`, `MailboxThreadContentResponseAllOfData`, `MailboxRawBodyResponseAllOfData` and `MailboxSubmissionEnvelopeRcptToInner` classes are removed. The matching setters accept the new types and the wire format is unchanged. See the [3.0 migration guide](https://github.com/Sendmux/sendmux-sdk/blob/main/packages/php/UPGRADING-3.0.md#upgrade-your-application).
- `MailboxMessage` gains `getMessageId()`, `getInReplyTo()` and `getReferences()` (`string[]`, angle brackets removed) and `getReplyTo()` (`MailboxAddress[]`), which the API already returned.
